...
const ProductVariation = () => {
  ...
  useEffect(() => {
    ...
    if (displayAttributCta && sellerId) {
      getData();
    }
  }, [router.isReady, id]);
  ...
  const getData = () => {
    const options = {
      method: "GET",
      url: `${process.env.NEXT_PUBLIC_BASE_URL}/global-attribute/all/${sellerId}`,
    };
    axios
      .create({
        headers: {
  ...
  const variatonSubmit = (values, { setSubmitting, resetForm }) => {
    ...
    axios
      ...
      .then(function (response) {
        // //
        router.push(
          `/admin/product/customizable-product/product-variations?id=${response.data.product._id}&scroll=save`
        );
        setSubmitting(false);
