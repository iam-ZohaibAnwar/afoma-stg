...
import { faAngleLeft, faImage } from "@fortawesome/pro-regular-svg-icons";
...
const CustomerOrderView = ({ cart }) => {
    ...
    try {
      ...
      axios
        ...
        .then(function (response) {
          ...
          router.push("/affiliate/my-orders");
        })
        .catch(function (error) {
          setCancelLoading(false);
  ...
  return (
    <>
      <Head>
        <title>My Order - Decentralized Marketplace</title>
        <meta
          name="description"
                      ...
                      <tr>
                        ...
                        {order?.cart?.map((cartItem, cartIndex) => (
                          <React.Fragment key={cartIndex}>
                            {cartItem.selectedVariations?.map(
                              (item, variationIndex) => (
                                <th
                                  key={`${cartIndex}-${variationIndex}`}
                                  className="w-[250px] py-3 px-4 font-medium"
                                >
                                  {item.attributeName}
                                </th>
                              )
                        ...
                        <tr key={index}>
                          ...
                          <td className="py-3 px-4 text-blue-950">
                            {data?.productData?.images[0]?.imageUrl ? (
                              <Image
                                src={data.productData.images[0].imageUrl}
                                alt={data.productData.images[0].altText}
                                height={45}
                                width={45}
                                className="shrink-0"
                              />
                        ...
                        ? `${order.currency || "CAD"}${order.cart
                            .filter(
                              (value, index, self) =>
                                ...
                                self.findIndex(
                                  (t) =>
                                    t.productData.seller?.id === value.productData.seller?.id
                                )
                            )
                            .reduce(
      ...
      <ConfirmModal
        ...
        bgClass="bg-red-500"
      />
      {reviewModalOpen && sellerId && productId && (
        <ReviewModal
