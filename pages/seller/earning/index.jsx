...
const Index = () => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .get(
        `${process.env.NEXT_PUBLIC_BASE_URL}/commission/seller/${userData?.sellerId}`
      )
      .then((res) => {
          ...
          <table className="text-sm w-full table-fixed border-separate border-spacing-0 bg-orange-50 ">
            <thead className="text-left text-sm font-medium text-orange-50 bg-blue-950 h-14">
              <tr>
                ...
                <th className="px-5 w-[170px]">Referral Amount</th>
              ...
              {loading ? (
                ...
                currentOrders?.map((orders, index) => (
                  <tr key={index} className="child">
                    ...
                    <td className="px-5 py-5 text-blue-950">
                      {orders?.payoutAmount
                        ? `CA$${parseFloat(orders?.payoutAmount).toFixed(2)}`
                        : "-"}
                    </td>
                    <td className="px-5 py-5 text-blue-950">
                      {orders?.referralAmount
                        ? `CA$${parseFloat(orders?.referralAmount).toFixed(2)}`
                        : "-"}
                    </td>
                    <td className="px-5 py-5 text-blue-950">
                    ...
                    <td className="px-5 py-5 text-blue-950">
                      {orders?.orderId?.cart.map((item) =>
                        item.productData.seller._id === orders?.seller?._id
                          ? item.productData.productName
                          : "-"
                      )}
                    </td>
